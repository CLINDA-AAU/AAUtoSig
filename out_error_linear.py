...
def out_error_AAUtoSig(train_df, validation_df, nsigs, true_sigs, loss_name, optimizer_name):
    ...
    _, out_error, _ = train_AAUtoSig(5000, model, train_df, validation_df, loss_name= loss_name, optimizer=optimizer, batch_size = params['batch_size'], do_plot = False, ES= False)
...
res = Parallel(n_jobs = 5)(delayed(performance_analysis)(100, 7, loss_name, optimizer_name) for i in range(n_sims))
print("analysis_done")
result = pd.DataFrame(res)
result.columns = ["NMF_perm", "outNMF", "AE_perm", "outAE" ]
print(result)
name = "Linear_"+ loss_name + "_ADAM_nsim:" + str(n_sims)
