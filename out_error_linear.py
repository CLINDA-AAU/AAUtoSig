...
def out_error_AAUtoSig(train_df, validation_df, nsigs, true_sigs, loss_name, optimizer_name):
    train = train_df.columns
    sigs = true_sigs.index 
    print(all(train == sigs))
    ...
    _, out_error, _ = train_AAUtoSig(5000, model, train_df, validation_df, loss_name= loss_name, optimizer=optimizer, batch_size = params['batch_size'], do_plot = True, ES= False)
    ...
    print(true_sigs.index)
    cos_AE = cosine_HA(signatures.T, true_sigs.T)[0]
    cos_mean = np.mean(cos_AE)
      
    return cos_mean, out_error

def performance_analysis(npatients, nsigs, loss_name, optimizer_name):
  ...
  cosineNMF_perm, outNMF = out_errorNMF(train_data_l, validation_data_l, nsigs, signatures_l, criterion = loss_name)
  cosineAE_perm, outAE = out_error_AAUtoSig(train_data_l, validation_data_l, nsigs, signatures_l, loss_name = loss_name, optimizer_name = optimizer_name)
...
res = Parallel(n_jobs = 10)(delayed(performance_analysis)(2000, 7, loss_name, optimizer_name) for i in range(n_sims))
result = pd.DataFrame(res)
result.columns = ["NMF_perm", "outNMF", "AE_perm", "outAE" ]
print(result)
name = "Lung_SCC_"+ loss_name + "_ADAM_nsim:" + str(n_sims)
